...
def generate_launch_description():
    ...
    # Gazebo config files
    gz_bridge_params = os.path.join(get_package_share_directory(package_name), 'config', 'gz_bridge_params.yaml') 

    #Launch Gazebo in ros_gz_sim package
    gz_sim = IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                [os.path.join(get_package_share_directory('ros_gz_sim'), 'launch', 'ros_gz_sim.launch.py')]),
                launch_arguments={
                    'use_sim_time': 'true'
    ...
    # Spawn a model node in ros_gz_sim package
    gz_spawn_model = Node(
            package='ros_gz_sim',
            executable='create',
            output='screen',
